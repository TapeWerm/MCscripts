...
for override in /etc/systemd/system/mc-backup@*.service.d/*.conf; do
	if [ -f "$override" ]; then
		sed -i -e 's/MCbackup\.sh/mc_backup\.sh/g' "$override"
		sed -i -e 's|MC/mc_backup\.sh|MCscripts/mc_backup\.sh|g' "$override"
		sed -i -e 's|MCscripts/mc_backup\.sh|MCscripts/bin/mc_backup\.sh|g' "$override"
		sed -i -e 's|MCscripts/mc_backup\.py|MCscripts/bin/mc_backup\.py|g' "$override"
	fi
done
for override in /etc/systemd/system/mc-rmbackup@*.service.d/*.conf; do
	if [ -f "$override" ]; then
		sed -i -e 's/%i_Backups/%i_backups/g' "$override"
		sed -i -e 's|java/%i_backups|java_backups/%i|g' "$override"
		sed -i -e 's|MC/backup_dir|MCscripts/backup_dir|g' "$override"
		sed -i -e "s/xargs -0d '\\\\n' ls -t/xargs -0rd '\\\\n' ls -t/g" "$override"
		sed -i -e "s/xargs -0rd '\\\\n' ls -t/xargs -rd '\\\\n' ls -t/g" "$override"
		sed -i -e "s/xargs -0d '\\\\n' rm -f/xargs -d '\\\\n' rm -f/g" "$override"
	fi
...
for override in /etc/systemd/system/mc@*.service.d/*.conf; do
	if [ -f "$override" ]; then
		sed -i -e 's/MCstop\.sh/mc_stop\.sh/g' "$override"
		sed -i -e 's|MC/mc_stop\.sh|MCscripts/mc_stop\.sh|g' "$override"
		sed -i -e 's|MCscripts/mc_stop\.sh|MCscripts/bin/mc_stop\.sh|g' "$override"
		sed -i -e 's|MCscripts/mc_stop\.py|MCscripts/bin/mc_stop\.py|g' "$override"
	fi
done
for override in /etc/systemd/system/mcbe-autoupdate@*.service.d/*.conf; do
	if [ -f "$override" ]; then
		sed -i -e 's|MCscripts/mcbe_autoupdate\.sh|MCscripts/bin/mcbe_autoupdate\.sh|g' "$override"
		sed -i -e 's|MCscripts/mcbe_autoupdate\.py|MCscripts/bin/mcbe_autoupdate\.py|g' "$override"
	fi
done
for override in /etc/systemd/system/mcbe-backup@*.service.d/*.conf; do
	if [ -f "$override" ]; then
		sed -i -e 's/MCBEbackup\.sh/mcbe_backup\.sh/g' "$override"
		sed -i -e 's|MC/mcbe_backup\.sh|MCscripts/mcbe_backup\.sh|g' "$override"
		sed -i -e 's|MCscripts/mcbe_backup\.sh|MCscripts/bin/mcbe_backup\.sh|g' "$override"
		sed -i -e 's|MCscripts/mcbe_backup\.py|MCscripts/bin/mcbe_backup\.py|g' "$override"
	fi
done
for override in /etc/systemd/system/mcbe-getzip.service.d/*.conf; do
	if [ -f "$override" ]; then
		sed -i -e 's/MCBEgetZIP\.sh/mcbe_getzip\.sh/g' "$override"
		sed -i -e 's|MC/mcbe_getzip\.sh|MCscripts/mcbe_getzip\.sh|g' "$override"
		sed -i -e 's|MCscripts/mcbe_getzip\.sh|MCscripts/bin/mcbe_getzip\.sh|g' "$override"
		sed -i -e 's|MCscripts/mcbe_getzip\.py|MCscripts/bin/mcbe_getzip\.py|g' "$override"
	fi
done
for override in /etc/systemd/system/mcbe-rmbackup@*.service.d/*.conf; do
	if [ -f "$override" ]; then
		sed -i -e 's/%i_Backups/%i_backups/g' "$override"
		sed -i -e 's|bedrock/%i_backups|bedrock_backups/%i|g' "$override"
		sed -i -e 's|MC/backup_dir|MCscripts/backup_dir|g' "$override"
		sed -i -e "s/xargs -0d '\\\\n' ls -t/xargs -0rd '\\\\n' ls -t/g" "$override"
		sed -i -e "s/xargs -0rd '\\\\n' ls -t/xargs -rd '\\\\n' ls -t/g" "$override"
		sed -i -e "s/xargs -0d '\\\\n' rm -f/xargs -d '\\\\n' rm -f/g" "$override"
