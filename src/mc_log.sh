...
# Exit if error
set -e
syntax='Usage: mc_log.sh SERVICE'
...
while [ "$1" != -- ]; do
	...
	--help|-h)
		echo "$syntax"
		echo 'Post Minecraft Java Edition server logs running in service to webhooks (Discord and Rocket Chat).'
		echo
...
instance=${service##*@}
discord_file=~/.mc_log/${instance}_discord.txt
if [ -f "$discord_file" ]; then
	chmod 600 "$discord_file"
fi
rocket_file=~/.mc_log/${instance}_rocket.txt
if [ -f "$rocket_file" ]; then
...
journalctl "_SYSTEMD_UNIT=$service.service" -fn 0 -o cat | while IFS='' read -r line; do
	...
	if connect=$(echo "$line" | grep -o '[^: ][^:]* joined the game'); then
		# Gamertags can have spaces if they're not leading/trailing/consecutive
		player=$(echo "$connect" | sed -Ee 's/([^: ][^:]*) joined the game/\1/')
		send "$player connected to $service"
	elif disconnect=$(echo "$line" | grep -o '[^: ][^:]* left the game'); then
		player=$(echo "$disconnect" | sed -Ee 's/([^: ][^:]*) left the game/\1/')
		send "$player disconnected from $service"
	elif kick=$(echo "$line" | grep -Eo 'Kicked [^:]+: .*'); then
		player=$(echo "$kick" | sed -Ee 's/Kicked ([^:]+): .*/\1/')
		reason=$(echo "$kick" | sed -Ee 's/Kicked [^:]+: (.*)/\1/')
