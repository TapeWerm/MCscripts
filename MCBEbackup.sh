...
syntax() {
	echo '`./MCBEbackup.sh $server_dir $sessionname [$backup_dir] [$tmux_socket]`'
}

if [ "$1" = -h ] || [ "$1" = --help ]; then
	echo Back up Minecraft Bedrock Edition server world running in tmux session.
	syntax
	echo 'Backups are ${world}_Backups_BE/$year/$month/${date}_$hour-$minute.zip in ~ or $backup_dir if applicable. $backup_dir is best on another drive.'
	exit
elif [ "$#" -lt 2 ]; then
